...
function virgin_add_theme() {
  ...
  register_nav_menus(
    array(
      'primary' => __('Réseaux sociaux', 'aladin'),
      'secondary' => __('Cta', 'aladin'),
    )
  );
...
function add_section_page_attributes(){
  add_post_type_support('section','page-attributes');
}
add_action('admin_init','add_section_page_attributes');
...
function oz_alter_wp_admin_bottom_left_text( $text ) {
    return sprintf( __( 'Merci de faire de l\'<a href="%s" title="L\'intermédiaire" target="_blank">intermédiaire</a> votre partenaire' ),'https://lintermediaire.be');
}
